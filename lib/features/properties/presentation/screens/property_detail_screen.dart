...
class _PropertyDetailScreenState extends ConsumerState<PropertyDetailScreen> {
  ...
  Future<void> _getRoute(Property p) async {
    ...
    try {
      if (_userPosition == null) await _getUserLocation();
      ...
      if (response.statusCode == 200) {
        ...
        final distanceM = (data['routes'][0]['distance'] as num).toDouble();
        final durationS = (data['routes'][0]['duration'] as num).toDouble();
        setState(() {
          _routePoints = coords.map((c) => LatLng(c[1] as double, c[0] as double)).toList();
          _distanceKm = distanceM / 1000;
          _durationMin = durationS / 60;
          _showRouteCard = true;
        });
      }
    } catch (_) {}
  ...
  LatLngBounds? _boundsFor(Property p) {
    if (_routePoints.isEmpty) return null;
    final pts = List<LatLng>.from(_routePoints);
    pts.add(LatLng(p.latitude, p.longitude));
    if (_userPosition != null) pts.add(LatLng(_userPosition!.latitude, _userPosition!.longitude));
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: p.type == 'rent' ? AppTheme.primary : AppTheme.accent, borderRadius: BorderRadius.circular(8)),
                  child: Text(p.typeLabel, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)),
                ),
                const SizedBox(width: 8),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                  decoration: BoxDecoration(color: AppTheme.primary.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
                  child: Text(p.propertyTypeLabel, style: TextStyle(color: AppTheme.primary, fontSize: 12, fontWeight: FontWeight.w600)),
                ),
              ]),
              ...
              Row(children: [
                ...
                Expanded(child: Text(p.address, style: const TextStyle(color: AppTheme.textSecondary, fontSize: 14))),
              ]),
              ...
              if (p.bedrooms > 0 || p.bathrooms > 0 || p.area > 0)
                Container(
                  ...
                  child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
                    if (p.bedrooms > 0)
                      Column(children: [
                        Icon(Icons.bed_outlined, color: AppTheme.primary, size: 24),
                        const SizedBox(height: 4),
                        Text(p.bedrooms.toString(), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                        const Text('Quartos', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                      ]),
                    if (p.bathrooms > 0)
                      Column(children: [
                        Icon(Icons.bathtub_outlined, color: AppTheme.primary, size: 24),
                        const SizedBox(height: 4),
                        Text(p.bathrooms.toString(), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                        const Text('WC', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                      ]),
                    if (p.area > 0)
                      Column(children: [
                        Icon(Icons.square_foot, color: AppTheme.primary, size: 24),
                        const SizedBox(height: 4),
                        Text(p.area.toStringAsFixed(0) + 'm2', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                        const Text('Area', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                      ]),
                  ]),
                ),
              const SizedBox(height: 20),
                      ...
                      children: [
                        ...
                        if (_routePoints.length > 1)
                          PolylineLayer(polylines: [
                            Polyline(points: _routePoints, strokeWidth: 7, color: Colors.white),
                          ]),
                        if (_routePoints.length > 1)
                          PolylineLayer(polylines: [
                            Polyline(points: _routePoints, strokeWidth: 5, gradientColors: [const Color(0xFF34C759), const Color(0xFFFFC107)]),
                          ]),
                        MarkerLayer(markers: [
                          if (_userPosition != null)
                            Marker(
                              ...
                              width: 26, height: 26,
                              child: Container(decoration: BoxDecoration(color: const Color(0xFF1A1A1A), shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 3))),
                            ),
                          Marker(
                            point: LatLng(p.latitude, p.longitude),
                            width: 26, height: 26,
                            child: Container(decoration: BoxDecoration(color: AppTheme.primary, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 3))),
                          ),
                        ]),
                      ],
                    ),
                    if (_showRouteCard && _durationMin > 0)
                      Positioned(
                        ...
                        child: Center(
                          child: Container(
                            ...
                            child: Text('chegada as ' + _arrivalTime(), style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Color(0xFF1A1A1A))),
                          ),
                        ),
                      ),
                    if (_showRouteCard && _durationMin > 0)
                      Positioned(
                        ...
                        child: Center(
                          child: Container(
                            ...
                            decoration: BoxDecoration(color: const Color(0xFFFF3B30), borderRadius: BorderRadius.circular(18), border: Border.all(color: Colors.white, width: 3), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.25), blurRadius: 8)]),
                            child: Text(_durationMin.round().toString() + ' min', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15)),
                          ),
                        ),
                      ),
                    Positioned(
                      ...
                      child: GestureDetector(
                        onTap: _loadingRoute ? null : () => _getRoute(p),
                        child: Container(
                          width: 42, height: 42,
                          decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.2), blurRadius: 6)]),
                          child: _loadingRoute
              ...
              if (_showRouteCard && _distanceKm > 0)
                Container(
                  margin: const EdgeInsets.only(top: 10),
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                  decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFFEEEEEE)), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 6)]),
                  child: Row(children: [
                    Icon(Icons.directions_car_filled, color: AppTheme.primary, size: 22),
                    const SizedBox(width: 10),
                    Text(_distanceKm.toStringAsFixed(1) + ' km', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                    const SizedBox(width: 10),
                    const Text('-', style: TextStyle(color: AppTheme.textSecondary)),
                    const SizedBox(width: 10),
                    Text(_durationMin.round().toString() + ' min de carro', style: const TextStyle(color: AppTheme.textSecondary, fontSize: 14)),
                  ]),
                ),
                      ...
                      child: GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Row(children: [
                            ...
                            const SizedBox(width: 8),
                            Text(
                              _selectedDate == null ? 'Data' : _selectedDate!.day.toString() + '/' + _selectedDate!.month.toString() + '/' + _selectedDate!.year.toString(),
  ...
  String _arrivalTime() {
    final now = DateTime.now().add(Duration(minutes: _durationMin.round()));
  ...
  Future<void> _pickDate(BuildContext context) async {
    final picked = await showDatePicker(
      ...
      builder: (ctx, child) => Theme(data: Theme.of(ctx).copyWith(colorScheme: ColorScheme.light(primary: AppTheme.primary)), child: child!),
    );
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 12),
          const Text('A rota ate ao imovel foi calculada no mapa.', style: TextStyle(color: Colors.grey, fontSize: 13)),
        ]),
        actions: [
  ...
  Future<void> _whatsapp(String phone, String title, String date, String time) async {
    ...
    final text = date.isEmpty
        ? 'Ola! Tenho interesse no imovel ' + title + ' que vi no Kubico.'
        : 'Ola! Quero confirmar a visita ao imovel ' + title + ' no dia ' + date + ' as ' + time + '. Vi no Kubico.';
